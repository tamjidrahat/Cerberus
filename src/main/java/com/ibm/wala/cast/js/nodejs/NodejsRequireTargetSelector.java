  ...
  public IMethod getCalleeTarget(CGNode caller, CallSiteReference site, IClass receiver) {
    ...
    if (calledMethod != null
      ...
      for (String target : targets) {
        // tamjid: we do not load any core nodejs module.
...
//        }
        try {
          ...
          //tamjid: if source module cannot be resolved (not a core library), continue
          if (sourceModule==null) {
            System.err.println("Target module:= "+target + " couldn't be resolved, ignoring module");
            continue;
          }
  ...
  public static SourceFileModule resolve(File rootDir, File dir, String target) throws IOException {
    //tamjid: we ignore to load core modules --> handle in prologue
    ...
    // tamjid: ignore libraries --> handle in prologue.
    SourceFileModule module = loadNodeModules(rootDir, dir, target);
    if (module != null) return module;
    //tamjid: if no module found, return null instead of throwing exception
    return null;
