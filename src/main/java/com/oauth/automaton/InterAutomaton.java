...
public class InterAutomaton extends Automaton {
	...
	// (slaveState1$slaveState2) -> Set(interState1, interState2, interEdge)
	protected Map<String, Set<SrcTgtEdgeWrapper>> STEMap = new HashMap<String, Set<SrcTgtEdgeWrapper>>();

	// maps master edge to Set(interState1, interState2, interEdge).
	// To find all interstates corresponding to master state (RegAutomaton)
	protected  Map<AutoEdge, Set<SrcTgtEdgeWrapper>> masterEdgeToInterSTE = new HashMap<AutoEdge, Set<SrcTgtEdgeWrapper>>();
...
//		System.out.println("Master ID: "+masterSt.getId()+ ", OutgoingStatesKeySet: " +masterSt.getOutgoingStatesKeySet());
				...
				if (masterNextEdge.isDotEdge()) {
					...
					for (AutoState slaveNxtSt : slaveSt
						...
						if (newInterSt != null) {
							...
							addMasterEdgeToInterSTE(masterNextEdge, new SrcTgtEdgeWrapper(interSt, newInterSt,
								newInterEdge));
					...
					// this line returns null for unknown reason :(
//					Set<AutoState> slaveNextStates = slaveSt
//							.outgoingStatesInvLookup(masterNextEdge);

					Set<AutoState> slaveNextStates = slaveSt.outgoingStatesInv.get(masterNextEdge);
	...
	public Set<SrcTgtEdgeWrapper> lookupByMasterEdge(AutoEdge masterEdge) {
		return masterEdgeToInterSTE.get(masterEdge);
	}

	protected boolean addMasterEdgeToInterSTE(AutoEdge masterEdge, SrcTgtEdgeWrapper ste) {
		Set<SrcTgtEdgeWrapper> steList = masterEdgeToInterSTE.get(masterEdge);

		if (steList == null) {
			// this initial size (4) can be tuned later on
			masterEdgeToInterSTE.put(masterEdge, steList = new HashSet<SrcTgtEdgeWrapper>(4));
		}
				...
				for (AutoEdge outEdge : s.outgoingStatesLookup(tgt)) {
					...
					if (outEdge.isDotEdge())
						...
						if(outEdge.getSrcStmt() != null)
							...
							b.append(outEdge.getId().toString());
						}
					}
					b.append("\"]\n");
				}
